    ...
    <div class="row px-xl-5">
        <div class="col-lg-3 d-none d-lg-block">
            <a class="btn d-flex align-items-center justify-content-between bg-primary w-100" data-toggle="collapse" href="#navbar-vertical" style="height: 65px; padding: 0 30px;">
                <h6 class="text-dark m-0"><i class="fa fa-bars mr-2"></i>{{__('navbar.categories')}}</h6>
            ...
            <nav class="navbar navbar-expand-lg bg-dark navbar-dark py-3 py-lg-0 px-0">
                ...
                <div class="collapse navbar-collapse justify-content-between" id="navbarCollapse">
                    <div class="navbar-nav mr-auto py-0">
                        <a href="{{route('home')}}" class="nav-item nav-link {{(Route::is('home')) ? 'active' : '' }}">{{__('navbar.home')}} </a>
                        <a href="{{route('website.shop')}}" class="nav-item nav-link {{(Route::is('website.shop')) ? 'active' : '' }}">{{__('navbar.shop')}}</a>
                        {{-- <a href="{{route('website.blog.index')}}" class="nav-item nav-link {{(Route::is('website.blog.index')) ? 'active' : '' }}">Blogs</a> --}}
                        <a href="{{route('website.categories')}}" class="nav-item nav-link {{(Route::is('website.categories')) ? 'active' : '' }}">{{__('navbar.categories')}}</a>
                        <a href="{{route('website.catalogs')}}" class="nav-item nav-link {{(Route::is('website.catalogs')) ? 'active' : '' }}">{{__('navbar.catalogs')}}</a>
                        <a href="{{ url('page/contact-us') }}" class="nav-item nav-link {{ Request::is('page/contact-us') ? 'active' : '' }}" >{{__('navbar.contact')}}</a>
                    </div>
